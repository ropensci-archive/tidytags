...
A core functionality of {tidytags} is to retrieve tweets data from a Twitter Archiving Google Sheet](https://tags.hawksey.info/) (TAGS). A TAGS tracker continuously collects tweets from Twitter, based on predefined search criteria and collection frequency. Here we offer **a brief overview related to getting setup with TAGS**, but be sure to read through the information on  the [TAGS landing page](https://tags.hawksey.info/get-tags/) for thorough instructions on getting started with TAGS.
