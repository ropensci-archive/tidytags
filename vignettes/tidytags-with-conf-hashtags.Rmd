...
**We note that there are alternate ways to access TAGS files**. One way is to simply download a CSV file from Google Sheets (in Google Sheets, navigate to File -> Download -> Comma-seperated values (CSV) to do so. Be sure to do so from TAGS' Archive page. Once this file is downloaded, you can read it like it like any other CSV using R, e.g.:

```{r, eval = FALSE}
example_df_all <- read_csv("my-downloaded-tags-file.csv")
```
...
example_unique_places <- dplyr::distinct(example_processed, screen_name, .keep_all = TRUE)
```

Then, we geocode the locations with the `geocode_tags()` function, which takes a tibble:
...
graph <- as_tbl_graph(example_edgelist) %>% 
