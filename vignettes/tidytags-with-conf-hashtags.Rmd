...
With a TAGS archive read into R, the next step is to use the **rtweet** package (via `rtweet::lookup_statuses()`) to access additional tweet data. The `tidytags::read_tags()` and `tidytags::pull_tweet_data()` functions work together to retrieve data from a TAGS source.
