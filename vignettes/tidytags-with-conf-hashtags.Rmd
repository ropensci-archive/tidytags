...
If you're having trouble setting up the Google Geocoding API Key, see the [Getting started with tidytags](https://bretsw.github.io/tidytags/articles/setup.html) vignette, **Pain Point #3**.

Once you've saved your Google Geocoding API Key to the `.Renviron` file, quit your R session and restart. The function `geocode_tags()` will work for you from now on. 
...
```{r geocode-data}
sample_unique_places <- dplyr::sample_frac(example_unique_places, 0.1)
...
In the dataset of `r nrow(example_processed)` tweets, there are `r nrow(get_replies(example_processed))` replies, `r nrow(get_retweets(example_processed))` retweets, `r nrow(get_quotes(example_processed))` quote tweets, and `nrow(get_mentions(example_processed))`.
